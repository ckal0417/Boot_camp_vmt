...
{
    public class CollaboratorService(ICollaboratorService) : ICollaboratorService
    {
        public GenericResponse<CollaboratorDto> Create(CreateCollaboratorRequest model)
        {
            var create = repository.Create(new AppDomain().Database.SqlServer.Entities.Collaborator{
                GitlabProfile
            })
